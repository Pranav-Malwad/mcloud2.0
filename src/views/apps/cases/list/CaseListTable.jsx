...
import AddCasesDrawer from './AddCasesDrawer'
...
const CaseListTable = ({ orderData }) => {

  const toLocaleString = (date) => {
    return new Date(date).toLocaleString(); // Adjust options as needed for specific formatting
  };
  // States
    ...
    () => [
      ...
      columnHelper.accessor('order', {
        ...
        cell: ({ row }) => (
          <Typography
            component={Link}
            href={getLocalizedUrl(`/apps/cases/details/${row.original.order}`, locale)}
            color='primary'
      ...
      columnHelper.accessor('caseOwner', {
        header: 'Case Owner',
        cell: ({ row }) => <Typography>{row.original.caseOwner}</Typography>
      }),
      columnHelper.accessor('contactName', {
        header: 'Contact Name',
        cell: ({ row }) => <Typography>{row.original.contactName}</Typography>
      }),
      columnHelper.accessor('caseOrigin', {
        header: 'Case Origin',
        cell: ({ row }) => <Typography>{row.original.caseOrigin}</Typography>
      }),
      ...
      columnHelper.accessor('internalComments', {
        header: 'Internal Comments',
        cell: ({ row }) => <Typography>{row.original.internalComments}</Typography>
      }),
      columnHelper.accessor('webEmail', {
        header: 'Web Email',
        cell: ({ row }) => <Typography>{row.original.webEmail}</Typography>
      }),
      columnHelper.accessor('webPhone', {
        header: 'Web Phone',
        cell: ({ row }) => <Typography>{row.original.webPhone}</Typography>
      }),
      columnHelper.accessor('createdDate', {
        header: 'Created Date',
        cell: ({ row }) => (
          <Typography>{toLocaleString(row.original.createdDate)}</Typography>
        )
      }),
      columnHelper.accessor('updatedDate', {
        header: 'Updated Date',
        cell: ({ row }) => (
          <Typography>{toLocaleString(row.original.updatedDate)}</Typography>
        )
      }),
