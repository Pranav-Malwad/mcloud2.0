...
import AddQuotesDrawer from './AddApolloDrawer'
...
import AddApolloDrawer from './AddApolloDrawer'
...
import AddTasksDrawer from './AddApolloDrawer'
...
const ApolloLeadsListTable = ({ apolloLeadData }) => {
  // States
    ...
    () => [
      ...
      columnHelper.accessor('order', {
        ...
        cell: ({ row }) => (
          <Typography
            component={Link}
            href={getLocalizedUrl(`/apps/leads/apollo_leads/details/${row.original.order}`, locale)}
            color='primary'
      ...
      columnHelper.accessor('fileName', {
        header: 'File Name',
        cell: ({ row }) => <Typography>{row.original.fileName}</Typography>
      }),
      columnHelper.accessor('totalRowCount', {
        header: 'Total Row Count',
        cell: ({ row }) => <Typography>{row.original.totalRowCount}</Typography>
      }),
      columnHelper.accessor('leadAcceptCount', {
        header: 'Lead Accept Count',
        cell: ({ row }) => <Typography>{row.original.leadAcceptCount}</Typography>
      }),
      columnHelper.accessor('leadRejectCount', {
        header: 'Lead Reject Count',
        cell: ({ row }) => <Typography>{row.original.leadRejectCount}</Typography>
      }),
      columnHelper.accessor('created', {
        header: 'Created',
        cell: ({ row }) => <Typography>{new Date(row.original.created).toLocaleDateString()}</Typography>
      }),
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-3'>
            ...
            <div className='flex flex-col'>
              <Typography
                component={Link}
                href={getLocalizedUrl('/apps/leads/apollo_leads/details/879861', locale)}
                color='text.primary'
...
export default ApolloLeadsListTable
