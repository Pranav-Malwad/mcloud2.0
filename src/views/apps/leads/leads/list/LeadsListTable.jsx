...
import AddLeadsDrawer from './AddLeadsDrawer'
// Third-party Imports
    ...
    () => [
      ...
      columnHelper.accessor('Lead', {
        ...
        cell: ({ row }) => (
          <Typography
            component={Link}
            href={getLocalizedUrl(`/apps/leads/leads/details/${row.original.order}`, locale)}
            color='primary'
      ...
      columnHelper.accessor('assignedDate', {
        ...
        cell: ({ row }) => (
          <Typography>{new Date(row.original.assignedDate).toDateString()}</Typography>
        )
      }),
      columnHelper.accessor('adoptedDate', {
        header: 'Adopted Date',
        cell: ({ row }) => (
          <Typography>{new Date(row.original.adoptedDate).toDateString()}</Typography>
        )
      }),
      columnHelper.accessor('nextFollowUpDate', {
        header: 'Next Follow-up Date',
        cell: ({ row }) => (
          <Typography>{new Date(row.original.nextFollowUpDate).toDateString()}</Typography>
        )
      }),
      columnHelper.accessor('leadStatus', {
        header: 'Lead Status',
        cell: ({ row }) => <Typography>{row.original.leadStatus}</Typography>
      }),
      columnHelper.accessor('leadOwner', {
        header: 'Lead Owner',
        cell: ({ row }) => <Typography>{row.original.leadOwner}</Typography>
      }),
      ...
      columnHelper.accessor('customerStatus', {
        ...
        cell: ({ row }) => <Typography>{row.original.customerStatus}</Typography>
      }),
      columnHelper.accessor('description', {
        header: 'Description',
        cell: ({ row }) => <Typography>{row.original.description}</Typography>
      }),
      columnHelper.accessor('serviceInterest', {
        header: 'Service Interest',
        cell: ({ row }) => <Typography>{row.original.serviceInterest}</Typography>
      }),
      columnHelper.accessor('uploadedDate', {
        header: 'Uploaded Date',
        cell: ({ row }) => <Typography>{new Date(row.original.uploadedDate).toDateString()}</Typography>
      }),
      columnHelper.accessor('updatedOn', {
        header: 'Updated On',
        cell: ({ row }) => <Typography>{new Date(row.original.updatedOn).toDateString()}</Typography>
      }),
      ...
      <Card>
        <CardContent className='flex justify-between max-sm:flex-col sm:items-center gap-4'>
          ...
          <div>
            ...
            <Button variant='contained' onClick={() => setAddUserOpen(!addUserOpen)} className='max-sm:is-full mr-4'>
              New
            </Button>
          </div>
        </CardContent>
