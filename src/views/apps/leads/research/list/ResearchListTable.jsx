...
import AddUserDrawer from './AddResearchDrawer'
import AddResearchDrawer from './AddResearchDrawer'
...
const AccountListTable = ({ researchData }) => {
  ...
  const columns = useMemo(
    () => [
          ...
          <div className='flex items-center gap-3'>
            ...
            <div className='flex flex-col items-start'>
              <Typography
                component={Link}
                color='text.primary'
                href={getLocalizedUrl(`/apps/leads/research/details/${row.original.customerId}`, locale)}
      ...
      columnHelper.accessor('sdr', {
        header: 'SDR',
        cell: ({ row }) => <Typography>{row.original.sdr}</Typography>
      }),
      ...
      columnHelper.accessor('createdDate', {
        header: 'Created Date',
        cell: ({ row }) => <Typography>{new Date(row.original.createdDate).toDateString()}</Typography>
      }),
      columnHelper.accessor('updatedDate', {
        header: 'Updated Date',
        cell: ({ row }) => <Typography>{new Date(row.original.updatedDate).toDateString()}</Typography>
      }),
